...
public struct SubprocessRunner: ProcessRunning {
    ...
    public func run(executable: String, arguments: [String], stdin: Data?, timeout: TimeInterval) throws -> ProcessResult {
        let process = Process()
        ...
        // Write stdin on its own background queue. A clipboard PNG comfortably
        // exceeds the kernel pipe buffer, so this write must never block the
        // poll loop below — that would defeat the watchdog entirely. Once the
        // child is terminated (the timeout path below), the write correctly
        // fails with EPIPE and that is expected, so it is not surfaced there.
        // On the normal-exit path below, though, an incomplete write means
        // the child got only part of its input, so that failure is recorded
        // and checked once the child has exited.
        let stdinFailure = UnsafeErrorBox()
        let stdinQueue = DispatchQueue(label: "clipssh.stdin")
        stdinQueue.async {
            if let stdin {
                do {
                    try inputPipe.fileHandleForWriting.write(contentsOf: stdin)
                } catch {
                    stdinFailure.value = error
                }
            }
            try? inputPipe.fileHandleForWriting.close()
        }

        // Read stderr on a background queue so a full pipe buffer cannot deadlock
        // the wait below. Read in chunks and keep only the first
        ...
        //
        // Progress is published to errorData after every chunk (not just at
        // the end) so that if the drain is later abandoned mid-flight (see
        // drainSemaphore below), whatever was actually captured is not lost.
        ...
        process.waitUntilExit()
        // The child itself has exited, but if a DESCENDANT it spawned
        // inherited the stderr pipe's write end (e.g. `sleep 30 &` before
        // `exit 0`), the read loop above never sees EOF and would block
        // forever — this is the same defect class as the stdin bug this
        // runner already guards against, just one step later. Bound the
        // wait by whatever remains of the overall watchdog deadline instead
        // of waiting unboundedly.
        let remaining = deadline.timeIntervalSinceNow
        if drainSemaphore.wait(timeout: .now() + max(remaining, 0)) == .timedOut {
            ...
            _ = drainSemaphore.wait(timeout: .now() + 0.5)
        }
        stdinQueue.sync {}
        ...
        if process.terminationStatus == 0, let failure = stdinFailure.value {
            throw failure
        }
